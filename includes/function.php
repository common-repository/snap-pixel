...
if (!class_exists('snapchat_pixel_functions')) {
    class snapchat_pixel_functions
    {
        ...
        public function __construct()
        {
            ...
            $this->ajax_addtocart = (isset($snapchat_pixel_code['ajax_addtocart']) ? $snapchat_pixel_code['ajax_addtocart'] : '');
            ...
            if ($this->snapchat_pixel_wooacces == "yes" && $this->addtocart == "checked" && $this->ajax_addtocart !== "checked" && $this->snapchat_pixel_wooexist() && !isset($_GET['wc-ajax'])) {
        ...
        public function snapchat_pixel_code_everywhere()
        {
            ...
            $product_id = $post->ID;
            $_product = wc_get_product($product_id);
            if ($post && $_product && is_product()) {
                $producted_id = $_product->get_id();
            }
            ?>
            <script type='text/javascript'>
                ...
                var item_ids = <?php if($producted_id){ ?> {'item_ids': ["<?php echo $producted_id; ?>"]} <?php } else {
                    echo "0";
                } ?>;
    ...
    if ($this->snapchat_pixel_wooexist() && $this->snapchat_pixel_wooacces == 'yes')
    {
        ...
        <script>
            snaptr('track', 'START_CHECKOUT', {
                'currency': "<?php echo $product_currency;?>", 'price': <?php echo $price;?>,
                'number_items': "<?php echo $num_items;?>", 'item_ids': ["<?php if ($product_id) {
                    echo $product_id;
                }?>"]
            });
        </script>
        ...
        <script>
            snaptr('track', 'PURCHASE', {
                'currency': "<?php echo $product_currency;?>", 'price': <?php echo $order_shipping_total;?>,
                'number_items': "<?php echo $num_items;?>", 'transaction_id': "<?php if ($order_id) {
                    echo $order_id;
                }?>", 'item_ids': ["<?php if ($content_ids) {
                    echo $content_ids;
                }?>"]
            });
        ...
        {
            if ($this->snapchat_pixel_wooexist() && !isset($_GET['wc-ajax'])) {
                global $post;
                $product_currency = get_woocommerce_currency();
                $product_price = "";
                ...
                <script>
                    jQuery('body').on('added_to_cart', function (e, h, w, button) {
                        ...
                        setTimeout(function () {
                            ...
                            snaptr('track', 'ADD_CART', {
                                'currency': "<?php echo $product_currency;?>", 'price': product_price,
        ...
        public function snapchat_pixel_code_search()
        {
            ...
            <script>
                snaptr('track', 'SEARCH', {
                    'search_string': "<?php if ( isset($_GET['s']) && esc_js( $_GET['s'] ) != '' ) {
                        echo esc_js( $_GET['s'] );
                    }?>"
                });
            </script>
